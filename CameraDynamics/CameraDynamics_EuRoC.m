...
if drawHistogram
    ...
    xlim([-0.05, 0.8]);
    str_title = strcat(datasetName, {' '}, 'sequence', {' '}, 'V2\_01\_easy');
    title(str_title);
    legend;
    set(gcf,'color','w');
end
...
if drawArcLengthCompare
    ...
    %> plot the results
    figure;
    frame_index = start_fr:end_fr;
    plot(frame_index, s_GT(1:end,1), 'bo-', 'DisplayName', 'ground truth');
    hold on;
    plot(frame_index, s_second_order(1:end,1), 'ro-', 'DisplayName', 'second order dynamic');
    hold on;
    plot(frame_index, s_third_order(1:end,1), 'go-', 'DisplayName', 'third order dynamic');
    legend;
    ylabel('arc-length');
    xlabel('frame index');
    set(gcf,'color','w');
...
if drawAvgErrHistogram && ~run_segment_wise
    sequenceName_wo_mav = erase(sequenceName, '/mav0/');
    sequenceName_wo_mav = strrep(sequenceName_wo_mav, '_', '\_');
    title_string = strcat(datasetName, {' '}, sequenceName_wo_mav);
    nBins = 40;
    figure;
...
%     [C, ~, ~, ~, ~, ~, alpha] = curve_generator(i, i+N-1, all_T, time_per_keyfr, N, piecewise_constant, 'exact', 'second_order');
...
%     [C, ~, ~, ~, ~, ~] = curve_generator(i, i+N-1, all_T, time_per_keyfr, N, piecewise_constant, '1st-order-approx', 'second_order');
%     p_curve_first_order = plot3(C(1,1:end), C(2,1:end), C(3,1:end), 'm*-');
%     err_curve = sum(abs(all_T(:,i:i+N-1) - C), 1);
%     fprintf('1st-order approximation curve error: %f\n', sqrt(sum(err_curve))/(N-1));
%     hold on;
%     
%     [C, ~, ~, ~, ~, ~] = curve_generator(i, i+N-1, all_T, time_per_keyfr, N, piecewise_constant, '2nd-order-approx', 'second_order');
...
if drawParams
    [~, speed, a0, curvature, torsion, jerk, ~] = curve_generator(start_fr, end_fr, all_T, time_per_keyfr, window_size, piecewise_constant, 'exact', 'third_order');
