function [opt_res, collect_gd_errors, collect_learning_rate, minErr_err_percentage] = ...
         self_GD_min_curve_err_common_tau_FF(gdParams, initParams, trajectory_pts, j_idx, err_type)
    % -- do gradient descent --
    ...
    for gd_iter = 1:gd_iterations
        ...
        [bihelixCurveFromModel, arcLength] = self_generateBihelixFromModel(k0, tau0, k1, tau0, T0, N0, B0, trajectory_pts, j_idx);
        
        % -- get gradients of curvature and torsion --
        [gradient_k0, gradient_tau0, gradient_k1, gradient_theta1, gradient_phi1, gradient_theta2, gradient_phi2, energy_val] = ...
        self_getGeometryParamsFFGradientsForBihelix_common_tau(k0, tau0, k1, theta1, phi1, theta2, phi2, arcLength, trajectory_pts, bihelixCurveFromModel, j_idx, err_type);
