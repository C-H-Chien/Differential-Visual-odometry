...
currentFileDirectory = '/oscar/data/bkimia/zqiwu/differential_pose_estimation/';
...
for start_fr = seq_start:seq_end
    ...
    for i = start_fr:start_fr+covisible_frames-1
        ...
        if i == start_fr
            ...
            if strcmp(featureType, 'vlfeat-SIFT') 
                ...
                covisible_gamma_tracks(:,1:2,cov_indx)  = f2_pts(:,1:2);
                
            else
                if ~strcmp(featureType, 'TAPIR') && ~strcmp(featureType, 'TEEATO')
                    %> delete unmatched features from prior frames
                    covisible_feature_tracks = covisible_feature_tracks(indexPairs(:,1),:,:);
                    covisible_gamma_tracks = covisible_gamma_tracks(indexPairs(:,1),:,:);
                    track_veridicality = track_veridicality(indexPairs(:, 1),:);
           ...
           %> Backward direction: find inliers of the previous frame using
            ...
            for feature_track_idx = 1: feature_track_num
                ...
                [correced_features, reproj_err] = fast_multiview_triangulation_mex(Feature_Track, K_, Rs, Ts, 0);
                %disp(correced_features)
                %disp(Feature_Track);
                %disp(feature_track_original);
                %disp(reproj_err);
                reprojected_feature_tracks(feature_track_idx, :, 1:cov_indx) = correced_features;

                %> Check if the returned reprojection errors are correct and make sense
                total_err = 0;
                total_idx = 0;
                for i = 1:cov_indx
                    if (abs(norm(Feature_Track(:,i) - correced_features(:,i)) - reproj_err(i)) < 1e-5 && abs(reproj_err(i)) < 20)
                        total_err = total_err + reproj_err(i);
            ...
            reprojection_error(cov_indx-1, 1) = tot_reproj_err / feature_track_num;


            min_num_inliers = length(common_inliers);
            if cov_indx==2
                current_bidirectional_inlier_ratio(cov_indx-1, window_idx) = 0;
            else
                current_bidirectional_inlier_ratio(cov_indx-1, window_idx) = min_num_inliers / collect_current_num_of_track_length(cov_indx-2,1);
            end
            collect_current_verdical_num_of_track_length(cov_indx-1, 1) = sum(track_veridicality(:, cov_indx)==1) / size(track_veridicality, 1);
    ...
    for i = 1:covisible_frames-1
        ...
        wr_ratio = current_bidirectional_inlier_ratio(i,window_idx);
...
if PARAMS.DRAW_TRACKS
    disp("drawing tracks");
    %self_drawColoredFeatureTracks_5_tracks_with_img(imgFileName(start_fr,1), covisible_frames, covisible_feature_tracks, reprojected_feature_tracks, -1);
    self_drawColoredFeatureTracks_5_tracks_with_img(imgFileName,start_fr, covisible_frames, covisible_feature_tracks, reprojected_feature_tracks, -1);
end
