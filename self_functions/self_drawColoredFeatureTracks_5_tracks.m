function self_drawColoredFeatureTracks_5_tracks(projectedImgName, covisible_frames, covisible_feature_tracks, reprojected_feature_tracks, plot_track_num)
    ...
    for i = 1:covisible_frames
        for k = 1:num_tracks
            ...
            if any(covisible_feature_tracks(k, :, i) ~= 0)
                plot(covisible_feature_tracks(k, 1, i), covisible_feature_tracks(k, 2, i), 'g+', 'Color', colors(k, :));
                if i >= 2
    ...
    for i = 1:covisible_frames
        for k = 1:num_tracks
            ...
            if any(reprojected_feature_tracks(k, :, i) ~= 0)
                plot(reprojected_feature_tracks(k, 1, i), reprojected_feature_tracks(k, 2, i), 'o-', 'Color', colors(k, :));
                if i >= 2
